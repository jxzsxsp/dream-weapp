...
Page({
  ...
  CalcOrderTotalPrice: function() {
    ...
    //判断红包余额是否比订单价格多
    if (e.data.OrderTotalPrice > e.data.usebalance){
      e.setData({
        isEnoughMoney:false
      });
    }else{
    ...
    i.data.InvoiceEnty && i.data.InvoiceEnty.Id, i.data.IsOpenInvoice && "增值税发票" == i.data.InvoiceTypeName && i.data.InvoiceId <= 0 ? wx.showModal({
      ...
      wx.request({
        ...
        data: (o = {
          ...
          usebalance: (i.data.isUseRedPacket && i.data.Muse && i.data.isEnoughMoney) ? i.data.usebalance : 0
